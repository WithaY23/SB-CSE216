...
(*UNUSED COMMENTED CODE; INITIAL THOUGHT PROCESS*)

(* 2 elements from list
If they are same, under f, combine under 1 list.
Else, create individual list for each.
Check the finalList, if the first element is the same to the first element/set of elements, add to that list.
Else, check the other list equivalency, add if valid. 
Else, add list element(s) to the main list individual of any other collection
*)
(*
let equivs = let rec subEquivs f lst finalList = match lst with
|x::y::tl -> if (f x y) then (*If the two elements are the same, they are in the same list*)
  (match finalList with
          |a::b::c::tail -> subEquivs f tl ([x;y] :: finalList) (*If the collection of lists has 3 or more elements, all the elements have their own list*)

          (*Collection has 2 elements, check for equivalency*)
          |a::b::tail -> if (f x (List.hd a)) then ( ) subEquivs f tl finalList) (*If it matches with first one, pair with first. Second  with second.
            If not either, then add as own list*)

          else(if f x (List.hd b) then ([x;y] @ b subEquivs f tl finalList) else subEquivs f tl ([x;y] :: finalList)) 
            
          (*Collection has 1 element*)
          |a::tail -> if (f x a) then ([x;y] @ a subEquivs f tl finalList) (*If it matches first, add to that list*)
          else subEquivs f tl ([x;y] :: finalList) (*Make own list*)
          |_ -> subEquivs f tl ([x;y] :: finalList) (*Collection is new, add element*)
...
|_ -> []
          ...
          in subEquivs f lst [];
(*Mutual Recursion*)
let finalListSort f originalLst newLst finalLst   = match final with
  |a::b::c::_-> subEquivs f originalLst (newLst :: finalLst) (*If the collection of lists has 3 or more elements, all the elements have their own list*)
  
  |a::b::tail->(*Collection has 2 elements, check for equivalency*)
    if (f (List.hd newLst) (List.hd a)) then (subEquivs f originalLst (function f lt x-> List.hd lt @ x)) (*If it matches with first one, pair with first. Second  with second.
    If not either, then add as own list*)
...
(*Helper for equivs, check the built list for an equivalent match
let rec equivalenceContainment f add lst = match lst with 
  |hd::tl ->  if (f (List.hd add) (List.hd hd)) then (add@hd)::tl  (*Found equivalence, add relevant elements to section, return the list*)
    else hd :: equivalenceContainment f add tl (*Didn't find it, keeping looping through till you do*)
  |_ ->[add];; (*Make the list their own list*)

let equivs f lst = let rec equivHelper f finalLst lst = match lst with
  |a::b::tl -> if f a b then  equivHelper f (equivalenceContainment f [a;b] finalLst) tl (*Equivalent, make into a list and check master list to helper*)
    else equivHelper f (equivalenceContainment f [a] (equivalenceContainment f [b] finalLst)) tl
  |[a]->equivalenceContainment f [a] finalLst
  |_->finalLst in equivHelper f [[]] lst;;
*)
...
(*Take a number and its max value, increment until it finds a prime, if it doesn't, return -1*)
(* (let primeCreator start maximum= let rec subPrime num max f = if num >max(* <-CHECK THIS PART, should be allowed to go up to max*)  then -1 
    else (if (f num) then num else subPrime (num+1) max f) in subPrime start maximum primeChecker;;
...
(*USING TOTAL INSTEAD OF :: *)
  (* let rec powerHelper total formed rest = match rest with (*Take our new head and add one element of tail, with all elements of tail*)
  |hd::tl-> powerHelper (total @ [hd]) (formed @ [hd]) tl
  |_-> total;;  *)
(*let rec powerset lst =
  match lst with
  | [] -> [[]]  (* The powerset of an empty list is a list containing an empty list *)
  | hd::tl ->
      let subsets = powerset tl in
      let rec add_head_to_subsets current_subsets new_subsets =
        match current_subsets with
        | [] -> new_subsets
        | subset::rest ->
            add_head_to_subsets rest (subset :: (hd :: subset) :: new_subsets)
      in
      add_head_to_subsets subsets []
(*
let rec powerset total lst = match lst with
|hd::tl-> [hd]::total :: powerset (hd::total) tl
|_->[];;

let rec powerset head tail = match tail with
|hd::tl-> [hd]:: ( (*For each element in tail, make an indivdual list with hd, reassign tail, and start powerset function again, powerset ele ::hd NEWTAIl *))
|_-> [];;
...
(*total- keep track of the big array we've built (USING  :: INSTEAD
formed- The current "branch" that we are on
rest- The rest that the "branch" has to available

*)
(*
let rec powersetHelper formed rest = match rest with (*Take our new head and add one element of tail, with all elements of tail*)
  |hd::tl-> formed:: (formed @ [hd]) :: powersetHelper (formed @ [hd]) tl
  |_-> [formed];; (*Doesn't have to go through, return head OR Possibly total *)

let rec powerset total lst = match lst with 
  |hd ::tl -> [hd] :: powerset (total @ (powersetHelper [hd] tl)) tl (*Attach hd by itself, attach the rest of the forms in powerHelper*)
  |_-> match total with 
    |hd::tl -> total
    |_ -> [[]];;
